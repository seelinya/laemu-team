...
async function seedAdmin() {
  ...
  console.log(`✓ Admin angelegt: ${email} / ${password}`)
}
...
async function seedTags() {
  ...
  console.log(`✓ ${data.length} Tags`)
}

async function seedContent() {
  const pieceCount = await db.piece.count()
  if (pieceCount > 0) {
    console.log('• Inhalte bereits vorhanden — überspringe Demo-Import.')
    return
  }
  ...
  console.log(`✓ ${instrumentDefs.length} Instrumente`)
  ...
  console.log(`✓ ${teacherDefs.length} Lehrpersonen`)

  // ── Kurse mit Modulen & Lektionen ──
  ...
  for (const p of pieceDefs) {
    ...
    if (p.rich) {
      // "Dr Alperose": vollständiges Beispiel mit Stimmen, Noten, Mixer etc.
  ...
  console.log(`✓ ${pieceDefs.length} Stücke (inkl. vollständigem Beispiel „Dr Alperose“)`)
  ...
  const wishes = [
    ...
    { title: 'Märzenschnee-Ländler', composer: '', votes: { 'Schwyzerörgeli (1. Stimme)': 17, 'Klarinette (1. Stimme)': 5 }, available: [] },
    { title: 'Luzerner Polka', composer: 'R. Suter', votes: { 'Klarinette (1. Stimme)': 28, 'Handorgel (1. Stimme)': 13, 'Bassgeige': 7 }, available: [] },
  ]
  ...
  console.log(`✓ ${wishes.length} Stückwünsche`)
}
